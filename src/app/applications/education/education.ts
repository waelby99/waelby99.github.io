...
export class Education implements OnInit {
  ...
  readonly selectedEducation = signal<EducationEntry | null>(null);
  ...
  private async loadEducation(): Promise<void> {
    try {
      ...
      const data = await response.json() as EducationEntry[];

      this.education.set(data);

      if (data.length > 0) {
        this.selectedEducation.set(data[0]);
      }
    } catch (error) {
      console.error('Could not load education:', error);
    } finally {
      this.loading.set(false);
    }
  }

  selectEducation(entry: EducationEntry): void {
